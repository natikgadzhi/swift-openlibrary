...
struct OpenLibrarySubjectTests {
  ...
  @Test func testOpenLibraryAPISubjectEndpoint() async throws {
    let session = SubjectSessionStub { request in
      ...
      #expect(
        components.queryItems?.first(where: { $0.name == "published_in" })?.value == "1500-1600")
    ...
    #expect(subject.works.count == 1)
    #expect(
      subject.coverImageURL?.absoluteString == "https://covers.openlibrary.org/b/id/1234567-L.jpg")
  }
